...
class LogStash::Filters::KubernetesMetadata < LogStash::Filters::Base
  ...
  def filter(event)
    ...
    if cached_metadata
      ...
      @logger.warn("Trying to get kubernetes file info, it was not cached");
      @logger.warn("kubernetes file info got: #{metadata}")
      ...
      @logger.warn("metadata within lookup_cache[path]: #{metadata}")
    end


    @logger.warn("metadata after unless lookup_cache[path] is: #{metadata}")
    @logger.warn("config after unless lookup_cache[path] is: #{config}")
